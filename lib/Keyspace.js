...
var getValidator = function (field) {
    ...
    return types.join("(") + types.map(function (o, i) {
            if (i == 0) {
                return "";
            }
            return ")";
...
var createDDL = function (cql, schema) {
    ...
    Object.keys(schema.children)
        .map(function (k) {
            return {name: k, meta: schema.children[k].meta[0]};
        })
        .forEach(function (field) {
            cql[field.meta.method](field.name, field.meta.arg);
...
var getChanges = function (schema, dbFields) {
    ...
    var expected = Object.keys(schema.children).map(function (k) {
        var res = schema.children[k].meta[0];
        res.validator = getValidator(schema.children[k].meta[0]);
        res.column_name = k;
        return res;
    });
    ...
    expected.forEach(function (field) {
        ...
        } else if (dbField.validator.indexOf(field.validator) !== 0) {
