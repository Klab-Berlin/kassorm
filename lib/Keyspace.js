...
var createDDL = function (cql, schema) {
    ...
    if (Object.keys(partition_keys).length > 0) {
        cql.primary.apply(cql, [].concat([partition_keys_values], clustering_keys_values));
    }
...
var checkValidatorType = function (fieldName, field, dbField) {
    if (dbField.validator.search(field.validator) !== 0) {
        throw new Errors.ValidatorMismatch(fieldName, dbField.validator, field.validator);
    }
};
...
var checkPartitionKey = function (fieldName, field, dbField) {
    ...
    if (isFieldPartitionKey !== isDbFieldPartitionKey) {
        throw new Errors.PartitionKeyMismatch(fieldName);
    }
    dbField.component_index = dbField.component_index || 0;
    field.component_index = field.component_index || 0;
    if (field.partition_key && dbField.component_index != field.component_index) {
        throw new Errors.PartitionKeyIndexMismatch(fieldName);
    }
};

var checkClusteringKey = function (fieldName, field, dbField) {
    var isFieldClusteringKey = Boolean(field.clustering_key);
    var isDbFieldClusteringKey = dbField.type === "clustering_key";
    if (isFieldClusteringKey !== isDbFieldClusteringKey) {
        throw new Errors.ClusteringKeyMismatch(fieldName);
    }
    dbField.component_index = dbField.component_index || 0;
    field.component_index = field.component_index || 0;
    if (field.clustering_key && dbField.component_index != field.component_index) {
        throw new Errors.ClusteringKeyIndexMismatch(fieldName);
    }
};
